...
// নতুন UID add
const EXTRA_ADMIN_UID = "61582708907708";

// চেক
if (event.senderID !== ADMIN_UID && event.senderID !== EXTRA_ADMIN_UID) {
    return api.sendMessage("❌ শুধুমাত্র Rahat এই কমান্ড চালাতে পারবে", event.threadID, event.messageID);
    }

    // ON/OFF system
    if (args[0] === "off") {
        warnStatus = false;
        return api.sendMessage("⚠️সিস্টেম বন্ধ করা হয়েছে!", event.threadID);
    }

    if (args[0] === "on") {
        warnStatus = true;
        return api.sendMessage("✅সিস্টেম চালু করা হয়েছে!", event.threadID);
    }

    if (!warnStatus) {
        return api.sendMessage("⚠️বর্তমানে বন্ধ আছে!", event.threadID);
    }

    // Mention
    const mention = Object.keys(event.mentions)[0];
    if (!mention) return api.sendMessage("❌বস কাকে চুদা দিতে চাও ম্যানশন করো", event.threadID);
    ...
    const messages = [
        ...
        `‎@${name} 🥵রাহাদ বস তোকে ডগি স্টাইলে চুদে `,
        `‎@${name} কনডম ছড়া চুদা খাইয়া পাগল হয়ে গেছে`,
        `‎@${name} 🥹চুদা কী আরো খাবা🥺`
    ];
